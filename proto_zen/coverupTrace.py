...
WATCH_PATHS = [r"C:\sensitiveFile"]
SENSITIVE_KEYWORD = "ask2025"
ALLOWED_USB_SERIALS = ["1234567890ABCDEF"]
COVERUP_EXTENSIONS = [".log", ".json", ".csv", ".rar", ".pf", ".onnx"]
recent_deletes = {}
...
class FileMonitorHandler(FileSystemEventHandler):
    def on_deleted(self, event):
        if not event.is_directory:
            ...
            if SENSITIVE_KEYWORD in file_name or any(file_name.endswith(ext) for ext in COVERUP_EXTENSIONS):
                print("\n[ALERT] Cover-up Attempt Detected")
                print(f"- Type: Sensitive File Deletion")
                print(f"- File Name: {os.path.basename(event.src_path)}")
                print(f"- Deleted From: {event.src_path}")
                print(f"- Timestamp: {timestamp}")
            elif SENSITIVE_KEYWORD in file_name:
...
if __name__ == "__main__":
    ...
    print("[*] Monitoring for sensitive file movement, deletion, and USB connection...\n")
